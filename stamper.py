...
from aether import generate_id, REQUIRED_FILES
...
def stamp_empty(name: str, path: str | Path, version: str = "1.0.0") -> Path:
    ...
    _write_json(capsule_path / "manifest.json", {
        "id": capsule_id, "name": name, "version": version, "created": datetime.now().isoformat()
    })
    _write_json(capsule_path / "definition.json", DEFAULT_DEFINITION)
    _write_json(capsule_path / "persona.json", DEFAULT_PERSONA)
    (capsule_path / "kb.md").write_text(DEFAULT_KB, encoding="utf-8")
    _write_json(capsule_path / "kg.jsonld", DEFAULT_KG)
...
def stamp_from_source(name: str, source_path: str | Path, output_path: str | Path, version: str = "1.0.0") -> Path:
    ...
    if suffix == ".md":
        (capsule_path / "kb.md").write_text(content, encoding="utf-8")
    elif suffix == ".jsonld":
        _write_json(capsule_path / "kg.jsonld", json.loads(content))
    elif suffix == ".json":
        data = json.loads(content)
        if "@graph" in data or "@context" in data or "@id" in data:
            _write_json(capsule_path / "kg.jsonld", data)
        else:
            _write_json(capsule_path / "definition.json", {**DEFAULT_DEFINITION, **data})
...
def validate_capsule(path: str | Path) -> dict:
    ...
    if not path.is_dir():
        return {"valid": False, "missing": list(REQUIRED_FILES.values()), "errors": ["Not a directory"]}
                ...
                if key == "manifest":
                    for field in ["id", "version"]:
                        if field not in data:
                            result["errors"].append(f"manifest.json missing '{field}'")
                            result["valid"] = False
...
def restamp(path: str | Path, new_version: str) -> Path:
    """Copy existing capsule to new folder with new version."""
    path = Path(path)
    ...
    _write_json(new_path / "manifest.json", {
        **manifest, "id": new_id, "version": new_version,
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        m1 = json.loads((cap1 / "manifest.json").read_text())
        m2 = json.loads((cap2 / "manifest.json").read_text())
